...
\begin{enumerate}
    ...
    \item \textbf{Client Requests}: The client sends a request to the server specifying the offset to the file to be downloaded and the chunk size. The client downloads the entire file by continuously varying the offset and requesting chunks of the specified size until the entire file is received.
    \item \textbf{Server Response}: The server reads the requested file in window and sends this back to the client.
\end{enumerate}
...
\subsection{Implementation Details}
\begin{enumerate}
    \item \textbf{Concurrency Model}: The server actively listens on its input port and responds to clients in FCFS fashion.
    \item \textbf{Client Requests}: Clients send a request to the server specifying the offset in the file to be processed and the chunk size. The client downloads the entire file in chunks of the specified size.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Reason}: This is due to the compounding effect as the number of clients increases. Increase in the number of clients leads to longer queue length. Therefore, the request the entered the queue most recently may end up waiting for a long time due to the increased length of the wait queue. This leads to increase in average service time of each client by the server.
    \end{enumerate}
    \item \textbf{Trend in Variance}
    \begin{enumerate}
        \item \textbf{Increase in Variance}: The variance in completion time also increases with the number of clients.
        \item \textbf{Reason}: With increasing length of the queue, the permutations of requests in the queue become largely diverse due to which we observe large variation in completion times.
    \end{enumerate}
...
\begin{enumerate}
    \item \textbf{Trend in Graph}
    \begin{enumerate}
        \item \textbf{Decrease}: As the number of back to back requests by a single client increases, the JFI remains relatively high and dips around c = 9
        \item \textbf{Reason}: Ideally, the fairness index should decrease as one client monopolizes server resources, leading to increased wait times for other clients. However, for small number of back to back requests, the server can still manage to handle requests from other clients in a timely manner, leading to a relatively high fairness index till c=9.
    \end{enumerate}
    \item \textbf{Aymptotic Behaviour of Jain's Fairness Index}
    \begin{enumerate}
        \item \textbf{For larger c values}: For larger number of back to back requests, the fairness index would decrease more significantly as the greedy client would dominate server resources, leading to increased wait times for other clients. For \textbf{$c = 10$ we obtained a JFI score of around 0.3}.
...
\begin{enumerate}
    ...
    \item \textbf{Client Requests}: Clients send requests to the server specifying the offset in the file to be processed and the chunk size. The client downloads the entire file in chunks of the specified size and by requesting for the same to the server.
    \item \textbf{Server Response}: The server processes each message received from the clients in a round-robin manner, counts the words, and sends the set of valid words back to the respective client.
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Reason}: The server now processes each clients request one at a time. Assuming equal request servicing time, round robin strategy forces only one request of a client to be handled at a time, hence all the clients have similar servicing time periods.
    \end{enumerate}
    \item \textbf{Limitations of Round Robin Scheduling}
    \begin{enumerate}
        \item \textbf{How can client still monopolize server resources}: If a client sends a very large size of request, it can still lead to increased wait times for other clients, even with round-robin scheduling, by keeping the server occupied with its request.
        \item \textbf{Reason}: This is because the server has to process each request in turn, and a large number of requests from one client can lead to increased overall processing time, affecting the responsiveness for other clients.
    \end{enumerate}
\end{enumerate}
