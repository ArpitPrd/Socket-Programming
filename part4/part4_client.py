...
"""
Part 4: Client implementation for Round-Robin server testing
Same as Part 3 client but can be used with Round-Robin server
"""
...
def run_concurrent_clients(config_file='config.json'):
    ...
    if completion_times:
        ...
        # Show how Round-Robin affects the greedy client
        if len(completion_times) >= 2:
            greedy_time = completion_times[0]  # First client is greedy
            normal_avg = sum(completion_times[1:]) / (len(completion_times) - 1)
            print(f"\nGreedy client completion time: {greedy_time:.3f} seconds")
            print(f"Normal clients average: {normal_avg:.3f} seconds")
            print(f"Greedy advantage ratio: {normal_avg/greedy_time:.2f}x")
